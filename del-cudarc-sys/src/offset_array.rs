...
pub fn sort(stream: cu::CUstream, p2idx: &CuVec<u32>, idx2q: &CuVec<u32>) {
    let np = p2idx.n - 1;
    let (func, _mdl) = crate::load_function_in_module(del_cudarc_kernel::OFFSET_ARRAY, "sort").unwrap();
    let mut builder = crate::Builder::new(stream);
    builder.arg_i32(np as i32);
    builder.arg_dptr(p2idx.dptr);
    builder.arg_dptr(idx2q.dptr);
    builder.launch_kernel(func, LaunchConfig::for_num_elems(np as u32)).unwrap();
}
...
pub fn inverse_map(stream: cu::CUstream, idx2jdx: &CuVec<u32>, jdx2idx_offset: &CuVec<u32>) {
    ...
    let num_jdx = jdx2idx_offset.n - 1;
    {
        ...
        builder.launch_kernel(func, LaunchConfig::for_num_elems(num_jdx as u32 + 1)).unwrap();
    }
}
